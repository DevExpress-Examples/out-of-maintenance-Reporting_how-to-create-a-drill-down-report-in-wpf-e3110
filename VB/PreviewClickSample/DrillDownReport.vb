	...
	Partial Public Class DrillDownReport
		...
		Private Sub xrChart1_BeforePrint(ByVal sender As Object, ByVal e As PrintEventArgs) Handles xrChart1.BeforePrint
			...
			chart.DataSource = ( _
					...
					Where p.CategoryID = CInt(Fix(GetCurrentColumnValue("CategoryID"))) _
					Select New With {Key .CategoryID = p.CategoryID, Key .ProductName = p.ProductName, Key .UnitsInStock = p.UnitsInStock, Key .myRow = p.UnitPrice * p.UnitsInStock}).ToArray()
